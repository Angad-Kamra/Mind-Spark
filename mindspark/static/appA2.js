(function() {
    ...
    function Za(a, b, f) {
        ...
        for (var h =
                1; h < a.length; h++) e.lineTo(a[h] * f, b[h] * f);
    ...
    function rb(a) {
        ...
        if (Ca) Da++, 30 < Da && (Da = 0);
            ...
            if (!ea) {
                ...
                ca & da[87] && (f[1] -= b, f[1] < f[2] &&
                    (f[1] = f[2]), f[1] > l[3] && f[1] < v - l[3] && (l[1] += b), 27 < l[1] && (l[1] = 27));
                ...
                if (Ta(f)) {
                    ...
                    W[0][3] = !0;
                    t = 30 * dt;
                    if (Ka = J > record) record = J, Ja.setItem("agar3sjs13k-record",
                        J), W[1][3] = !0, W[2][3] = !0;
                    ja = 0
                }
            ...
            for (a =
                0; a < oa.length; a++) b = oa[a], b[0] += Math.cos(b[2]) * w(3, 2), b[1] += Math.sin(b[2]) * w(3, 2), 0 > --b[3] && oa.splice(a, 1);
            0 < ba && (ba -= .1);
            ia = {};
            for (a = F.length - 1; 0 <= a; a--) a: if (b = F[a], q = a, 0 >= b[6]) {
                ...
                if (10 > b[5]) {
                    ...
                    4 != b[5] ? (b[0] += Math.cos(b[9]) * t * b[12], b[1] += Math.sin(b[9]) * t * b[12]) : (1 > b[13][6] && (b[10] *= .99), b[15] = 2 * n * (-Math.cos(b[16]) +
                        1.2), b[16] += t / 200, b[0] = b[13][0] + Math.cos(b[9]) * b[15], b[1] = b[13][1] + Math.sin(b[9]) * b[15])
                } else {
                    b[9] -= t;
                    12 <= b[5] && (b[3] += b[12] * t);
        ...
        if (Ca) {
            ...
            e.stroke();
            va ? (C("controls", 400, 130, 12, [0, 16]), C("move             awsd", 400, 251, 12, [0, 0]), C("fire       left click", 400, 290, 12, [0, 0]), C("warptime  right click", 400, 330, 12, [0, 0])) : (C("winners don't use drugs", 401,
                50, 9, [0, 0]), C("evil glitch", 400, 270 - 50 * N, 30 * (1 + N), [0, 9, 0, 9]));
            ...
            for (a = 0; a < F.length; a++)
                if (h = F[a], !(20 > h[0] + l[0] || h[0] + l[0] > R - 20 || 20 > h[1] + l[1] || h[1] + l[1] > H - 20)) {
                    ...
                    if (10 > h[5]) p(h[5] + 24), p(-1, 2, 2), e.rotate(h[9]), Za(h[7], h[8], h[2]), e.rotate(-h[9]);
                        ...
                        e.stroke();
                        e.rotate(h[3]);
                        for (x = 0; 6 > x; x++) X = ga[2 * x], G = ga[2 * x + 1], e.beginPath(), e.arc(4 * X * m, 4 * G * m, m, 0, 2 * Math.PI, !1), e.stroke(), e.beginPath(), e.arc(2 * X * m, 2 * G * m, m, 0, 2 * Math.PI, !1), e.stroke(), e.beginPath(), e.moveTo(X * m * 4, G * m * 4), e.lineTo(4 * m * ga[(2 * x + 2) % 12], 4 * m * ga[(2 * x + 3) % 12]), e.lineTo(4 *
                            m * ga[(2 * x + 6) % 12], 4 * m * ga[(2 * x + 7) % 12]), e.moveTo(X * m * 2, G * m * 2), e.lineTo(2 * m * ga[(2 * x + 2) % 12], 2 * m * ga[(2 * x + 3) % 12]), e.lineTo(2 * m * ga[(2 * x + 6) % 12], 2 * m * ga[(2 * x + 7) % 12]), e.stroke();
                        e.beginPath();
                        e.rotate(-h[3])
                    } else
            ...
            ea ? (p(22, 1), e.fillRect(0, 0, v, v), za &&
                C("evil mode", 400, 80, 22, [0, 16]), Ka ? (C("-new record-", 400, 240, 22, [10, 18]), C("-share it-", 400, 400, 14, [24, 18])) : C("game over", 400, 240, 20, [0, 13]), C(J.toFixed(0), 400, 160, Ka ? 20 : 16, [0, 9])) : (C(6 < wave ? "evil" : wave + "/6", 400, 60, 9, [0, 3]), C(J.toFixed(0), 750, 60, 18, [32, 9], 1), C(J > record ? "record" : record.toFixed(0), 750, 110, 9, [24, 3], 1));
            e.restore()
        }
        Oa || Ea || (e.save(), e.beginPath(), p(-1, 2, 2), e.translate(B[0], B[1]), p(6), e.translate(-10, -10), Xa(10, 20), e.stroke(), e.closePath(), e.restore());
        e.save();
        for (a = 0; a < W.length; a++) b =
            W[a], b[3] && (q = b[7] ? 14 : b[4], p(q), p(-1, 2, 2), e.strokeRect(b[0], b[1], b[2], 42), C(b[5], b[0] + b[2] / 2, b[1] + 9, 12, [0, q]));
        e.restore();
    ...
    var Zb = 440 * Math.pow(Math.pow(2,
            1 / 12), -9),
        $b = /^[0-9.]+$/,
        Mb = /\s+/,
        ac = /(\d+)/,
    ...
    A.prototype.createOscillator = function() {
        ...
        this.K ? this.A.setPeriodicWave(this.C.createPeriodicWave.apply(this.C,
            this.K)) : this.A.type = this.W || "square";
    ...
    S([1, .0076, .66, , , .09, , .96, .32, .1, .97, -1, ,
        .0615, -.1587, 1, , -.02, .83, .12, .23, .0231, -.02, .96
    ]);
    ...
    for (var W = [
            ...
            [120, 460, 250, !1, 11, "twitter", !1, !1, function() {
                window.open("https://twitter.com/home?status=" +
                    encodeURIComponent(pb() + "http://js13kgames.com/entries/evil-glitch"))
            }],
